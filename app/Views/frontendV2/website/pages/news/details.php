        ...
        <div class="container mx-auto px-4 max-w-4xl">
            ...
            <nav class="mb-8" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2 text-sm">
                    ...
                    <li>
                        <a href="<?= base_url('news') ?>" class="text-slate-600 hover:text-primary transition-colors">News</a>
                    </li>
                    <li>
                        <i data-lucide="chevron-right" class="icon-xs text-slate-400"></i>
                    </li>
                    <li class="text-primary" aria-current="page">
                        <span><?= esc($blogPost->title) ?></span>
                    </li>
                </ol>
            </nav>
            ...
            <header class="mb-12">
                ...
                <div class="flex items-center text-slate-500 text-sm mb-6">
                    <?php if (!empty($blogPost->category_name)): ?>
                        <span class="bg-secondary text-white px-3 py-1 rounded-full text-xs font-medium mr-3">
                            <?= esc($blogPost->category_name) ?>
                        </span>
                    <?php endif; ?>
                    <span><?= date('F j, Y', strtotime($blogPost->published_at)) ?></span>
                ...
                <!-- Article Title -->
                <div class="mb-8">
                    <h1 class="text-3xl md:text-4xl lg:text-5xl font-bold text-slate-800 leading-tight max-w-4xl mx-auto">
                        <?= esc($blogPost->title) ?>
                    </h1>
                </div>
                ...
                <!-- Author Information -->
                <div class="flex items-center">
                    <div class="w-12 h-12 rounded-full overflow-hidden mr-4 bg-gradient-to-br from-primary to-secondary flex items-center justify-center">
                        <span class="text-white font-semibold text-lg">
                            <?= strtoupper(substr($authorName, 0, 1)) ?>
                        </span>
                    </div>
                    <div class="text-left">
                        <p class="font-medium text-slate-800 text-lg">By <?= esc($authorName) ?></p>
                        <p class="text-sm text-slate-500">Published on <?= date('F j, Y', strtotime($blogPost->published_at)) ?></p>
                    </div>
                </div>
            </header>
            ...
            <div class="prose max-w-none">
                ...
                <!-- Main Content -->
                <div class="blog-content text-slate-700 leading-relaxed">
                    <?= $blogPost->content ?>
                </div>
            </div>
            ...
            <?php if (!empty($additionalImages) && count($additionalImages) > 0): ?>
                <div class="my-12">
                    <h3 class="text-xl font-bold text-slate-800 mb-6">Image Gallery</h3>
                        ...
                        <?php foreach ($additionalImages as $image): ?>
                            <div class="group relative overflow-hidden rounded-lg shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer">
                                ...
                                <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end">
                                    <p class="text-white text-sm p-4 truncate w-full">
            ...
            <footer class="mt-16 pt-8 border-t border-slate-200">
                <!-- Tags -->
                <div class="mb-8">
                    <h3 class="text-sm font-semibold text-slate-600 uppercase tracking-wider mb-4">Tags</h3>
                    <div class="flex flex-wrap gap-2">
                        <a href="#" class="bg-slate-200 hover:bg-secondary hover:text-white text-slate-700 px-3 py-1 rounded-full text-sm transition-colors">#WASH</a>
                        <a href="#" class="bg-slate-200 hover:bg-secondary hover:text-white text-slate-700 px-3 py-1 rounded-full text-sm transition-colors">#Policy</a>
                        <a href="#" class="bg-slate-200 hover:bg-secondary hover:text-white text-slate-700 px-3 py-1 rounded-full text-sm transition-colors">#Kenya</a>
                        <a href="#" class="bg-slate-200 hover:bg-secondary hover:text-white text-slate-700 px-3 py-1 rounded-full text-sm transition-colors">#Water</a>
                        <a href="#" class="bg-slate-200 hover:bg-secondary hover:text-white text-slate-700 px-3 py-1 rounded-full text-sm transition-colors">#Sanitation</a>
                        <a href="#" class="bg-slate-200 hover:bg-secondary hover:text-white text-slate-700 px-3 py-1 rounded-full text-sm transition-colors">#SDG6</a>
                    </div>
                </div>
                
                <!-- Share Buttons -->
                <div class="mb-12">
                    <h3 class="text-sm font-semibold text-slate-600 uppercase tracking-wider mb-4">Share this article</h3>
                    <div class="flex space-x-4">
                        <a href="#" data-platform="facebook" class="share-btn w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 transition-colors" title="Share on Facebook">
                            <i class="fab fa-facebook-f"></i>
                        </a>
                        <a href="#" data-platform="twitter" class="share-btn w-10 h-10 rounded-full bg-blue-400 text-white flex items-center justify-center hover:bg-blue-500 transition-colors" title="Share on Twitter">
                            <i class="fab fa-twitter"></i>
                        </a>
                        <a href="#" data-platform="linkedin" class="share-btn w-10 h-10 rounded-full bg-blue-700 text-white flex items-center justify-center hover:bg-blue-800 transition-colors" title="Share on LinkedIn">
                            <i class="fab fa-linkedin-in"></i>
                        </a>
                        <a href="https://wa.me/?text=<?= urlencode(esc($blogPost->title) . ' - ' . current_url()) ?>" target="_blank" class="w-10 h-10 rounded-full bg-green-500 text-white flex items-center justify-center hover:bg-green-600 transition-colors" title="Share on WhatsApp">
                            <i class="fab fa-whatsapp"></i>
                        </a>
                        <a href="#" data-platform="copy" class="share-btn w-10 h-10 rounded-full bg-slate-200 text-slate-700 flex items-center justify-center hover:bg-slate-300 transition-colors" title="Copy link">
                                    ...
                                    <div class="relative h-40 overflow-hidden">
                                        ...
                                        <?php else: ?>
                                            <div class="w-full h-full bg-gradient-to-br from-primary/20 to-secondary/20 flex items-center justify-center">
                                                <i data-lucide="image" class="icon-xl text-primary/50"></i>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                    <div class="p-4">
                                        ...
                                        <?php endif; ?>
                                        <h4 class="text-lg font-bold text-slate-800 group-hover:text-secondary transition-colors mb-2 line-clamp-2">
