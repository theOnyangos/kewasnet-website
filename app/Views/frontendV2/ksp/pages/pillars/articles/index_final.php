            ...
            <div class="grid lg:grid-cols-4 gap-8">
                ...
                <aside class="lg:col-span-1">
                    <div class="bg-white rounded-xl shadow-sm border p-6 sticky top-8">
                            ...
                            <div class="space-y-2">
                                ...
                                <?php if (!empty($categories)): ?>
                                    <?php foreach ($categories as $category): ?>
                                        <label class="flex items-center space-x-3 cursor-pointer">
                                            <input type="radio" name="category_filter" value="<?= $category->id ?>" 
                                                   <?= ($filters['category'] ?? '') == $category->id ? 'checked' : '' ?>
                                                   class="text-primary focus:ring-primary" onchange="filterByCategory(<?= $category->id ?>)">
                                            <span><?= esc($category->name) ?> (<?= $category->resource_count ?? 0 ?>)</span>
                                        </label>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <?php if (!empty($documentTypes)): ?>
                                    <?php foreach ($documentTypes as $docType): ?>
                                        <label class="flex items-center space-x-3 cursor-pointer">
                                            <input type="radio" name="document_type_filter" value="<?= $docType->id ?>" 
                                                   <?= ($filters['document_type'] ?? '') == $docType->id ? 'checked' : '' ?>
                                                   class="text-primary focus:ring-primary" onchange="filterByDocumentType(<?= $docType->id ?>)">
                                            <span>
                                                <i class="<?= $docType->icon ?? 'fa fa-file' ?> w-4 h-4 mr-1"></i>
                                                <?= esc($docType->name) ?>
                                            </span>
                                        </label>
                                    <?php endforeach; ?>
                        ...
                        <!-- Reset Filters -->
                        <button onclick="resetFilters()" class="w-full bg-gray-100 text-gray-700 py-3 rounded-lg hover:bg-gray-200 transition-colors font-medium">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border p-6 mb-6">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                            ...
                            <div class="mt-4 md:mt-0">
                                <select id="sort" name="sort" class="border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-transparent">
                    ...
                    <?php if (!empty($resources)): ?>
                        <div class="grid md:grid-cols-2 gap-6 mb-8">
                            <?php foreach ($resources as $resource): ?>
                                <article class="bg-white rounded-xl shadow-sm border hover:shadow-md transition-shadow">
                                    <div class="p-6">
                                        <!-- Resource Header -->
                                            ...
                                            <div class="flex-1">
                                                <div class="flex items-center mb-2">
                                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary/10 text-primary">
                                                        <i class="<?= $resource->document_type_icon ?? 'fa fa-file' ?> w-3 h-3 mr-1"></i>
                                                ...
                                                <p class="text-gray-600 text-sm line-clamp-3 mb-4">
                                                    <?= esc($resource->description ?? 'No description available.') ?>
                                                </p>
                                            </div>
                                            <button onclick="bookmarkResource(<?= $resource->id ?>)" class="text-gray-400 hover:text-primary transition-colors ml-4">
                                                ...
                                                <div class="flex items-center -space-x-2">
                                                    <?php foreach (array_slice($resource->contributors, 0, 3) as $contributor): ?>
                                                        <div class="w-8 h-8 rounded-full bg-gray-200 border-2 border-white flex items-center justify-center text-xs font-medium">
                                                            <?= strtoupper(substr($contributor->name ?? 'U', 0, 1)) ?>
                                                        </div>
                                                    <?php endforeach; ?>
