            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php foreach ($forumSubscriptions as $forum): ?>
                    <a href="<?= base_url('ksp/networking-corner-forum-discussion/' . $forum['slug']) ?>" 
                       class="bg-white rounded-lg shadow-sm border border-slate-200 hover:shadow-md transition-all duration-300 overflow-hidden group">
                        <div class="p-6">
                            <div class="flex items-start gap-4 mb-4">
                                <div class="<?= $forum['color'] ?? 'bg-primary' ?> p-3 rounded-lg text-white group-hover:scale-110 transition-transform">
                                    <i data-lucide="<?= $forum['icon'] ?? 'users' ?>" class="w-6 h-6"></i>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <h3 class="font-bold text-dark text-lg mb-1 group-hover:text-primary transition-colors truncate">
                                        <?= esc($forum['name']) ?>
