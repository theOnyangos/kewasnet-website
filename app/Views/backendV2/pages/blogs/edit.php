...
<?= $this->section('content') ?>
    <main class="flex-1 overflow-y-auto">
        <?= view('backendV2/partials/page_banner', [
            ...
            'pageDescription' => 'Update your blog post details',
            'breadcrumbs' => [
                ['label' => 'Blogs', 'url' => base_url('auth/blogs/blogs')],
                ['label' => 'Edit']
            ],
            'bannerActions' => '<a href="' . base_url('auth/blogs/blogs') . '" class="inline-flex items-center px-4 py-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg text-white hover:bg-white/20 transition-colors">
        ...
        <div class="bg-white rounded-b-xl shadow-sm max-w-full blog-form-container mx-5">
            <!-- Main Form -->
            <form class="p-6 space-y-6" id="editBlogForm" method="POST" action="<?= site_url('auth/blogs/' . $blogPost['id'] . '/edit') ?>" enctype="multipart/form-data">
                <!-- Title Field -->
                <div>
                    <label for="title" class="block text-sm font-medium text-dark mb-1">Post Title <span class="text-red-500">*</span></label>
                    <input type="text" id="title" name="title" value="<?= esc($blogPost['title']) ?>" class="w-full px-4 py-3 border border-borderColor rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent" placeholder="Enter your post title">
                ...
                <div>
                    ...
                    <div class="flex w-full">
                        ...
                        <input type="text" id="blogSlug" name="slug" value="<?= esc($blogPost['slug']) ?>" data-manual="true" class="flex-1 min-w-0 px-4 py-3 rounded-none rounded-r-md border border-borderColor focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent" placeholder="url-friendly-slug">
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        <?php foreach ($categories as $category): ?>
                            <label class="inline-flex items-center p-3 border border-borderColor rounded-lg hover:bg-gray-50 cursor-pointer transition-colors has-[:checked]:border-secondary has-[:checked]:bg-secondaryShades-100">
                                <input type="radio" name="category" value="<?= $category['id'] ?>" <?= ($category['id'] == $blogPost['category_id']) ? 'checked' : '' ?> class="h-4 w-4 text-secondary focus:ring-secondary border-borderColor">
                ...
                <div>
                    ...
                    <?php if (!empty($blogPost['featured_image'])): ?>
                        <div id="existing-featured-image" class="mb-4">
                            <p class="text-sm text-gray-600 mb-2">Current Featured Image</p>
                            <div class="inline-block relative group">
                                <img src="<?= $blogPost['featured_image'] ?>"
                                     alt="Featured Image"
                                     class="max-w-sm max-h-48 rounded-lg border-2 border-gray-200"
                                     onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\'bg-gray-100 rounded-lg border-2 border-gray-300 p-8 text-center max-w-sm\'><i data-lucide=\'image-off\' class=\'w-12 h-12 mx-auto text-gray-400 mb-2\'></i><p class=\'text-sm text-gray-500\'>Image not found</p><p class=\'text-xs text-gray-400 mt-1\'><?= esc($blogPost['featured_image']) ?></p></div>'; lucide.createIcons();">
                                <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all rounded-lg flex items-center justify-center">
                                    <button type="button" onclick="removeExistingFeaturedImage()" class="opacity-0 group-hover:opacity-100 bg-red-500 text-white px-3 py-1 rounded-md text-sm hover:bg-red-600 transition-all">
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-slate-600">
                                <label for="featured_image" class="relative cursor-pointer bg-white rounded-md font-medium text-secondary hover:text-secondaryShades-600 focus-within:outline-none">
                                    <span><?= !empty($blogPost['featured_image']) ? 'Upload a new featured image' : 'Upload a featured image' ?></span>
                    ...
                    <!-- Hidden input to track featured image removal -->
                    <input type="hidden" id="remove-featured-image" name="remove_featured_image" value="0">
                </div>
                ...
                <div>
                    ...
                    <?php if (!empty($additionalImages)): ?>
                        <div class="mb-4">
                            ...
                            <div id="existing-images" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                                <?php foreach ($additionalImages as $image): ?>
                                    <div class="existing-image-item relative group" data-image-id="<?= $image['id'] ?>">
                                        <img src="<?= $image['file_path_display'] ?? base_url($image['file_path']) ?>" alt="<?= esc($image['original_name']) ?>" class="w-full h-32 object-cover rounded-lg border-2 border-gray-200">
                                        <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all rounded-lg flex items-center justify-center">
                                            <button type="button" onclick="deleteExistingImage('<?= $image['id'] ?>')" class="opacity-0 group-hover:opacity-100 bg-red-500 text-white px-3 py-1 rounded-md text-sm hover:bg-red-600 transition-all">
                                                <i data-lucide="trash-2" class="w-4 h-4 inline mr-1"></i>
                                                Delete
                                            </button>
                                        </div>
                                        <p class="text-xs text-gray-500 mt-1 truncate"><?= esc($image['original_name']) ?></p>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    ...
                    <div class="border-2 border-dashed border-borderColor rounded-lg p-6 text-center">
                        ...
                        <label for="additional-images" class="cursor-pointer">
                            ...
                            <p class="text-sm text-slate-600">Drag & drop new images or <span class="text-secondary">Click to browse</span></p>
                ...
                <div>
                    ...
                    <div class="relative">
                        <textarea id="excerpt" name="excerpt" rows="3" maxlength="1000" class="w-full px-4 py-3 border border-borderColor rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent resize-none" placeholder="A short summary of your post (appears in listings)"><?= esc($blogPost['excerpt'] ?? '') ?></textarea>
                ...
                <div>
                    ...
                    <div class="rounded-lg">
                        <textarea id="content" name="content" required class="summernote-editor w-full px-4 py-3 focus:outline-none" placeholder="Write your post content here..."><?= $blogPost['content'] ?? '' ?></textarea>
                    </div>
                </div>
                ...
                <div class="pt-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="meta_title" name="meta_title" value="<?= esc($blogPost['meta_title'] ?? '') ?>" class="w-full px-4 py-3 border border-borderColor rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent" placeholder="Title for search engines">
                        ...
                        <div>
                            ...
                            <textarea id="meta_description" name="meta_description" rows="3" class="w-full px-4 py-3 border border-borderColor rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent" placeholder="Description for search engines"><?= esc($blogPost['meta_description'] ?? '') ?></textarea>
                        ...
                        <div>
                            ...
                            <input type="text" id="meta_keywords" name="meta_keywords" value="<?= esc($blogPost['meta_keywords'] ?? '') ?>" class="w-full px-4 py-3 border border-borderColor rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent" placeholder="Comma-separated keywords">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="flex flex-col space-y-3">
                                <label class="inline-flex items-center p-3 border border-borderColor rounded-lg hover:bg-gray-50 cursor-pointer transition-colors has-[:checked]:border-secondary has-[:checked]:bg-secondaryShades-100">
                                    <input type="radio" name="status" value="draft" <?= ($blogPost['status'] === 'draft') ? 'checked' : '' ?> class="h-4 w-4 text-secondary focus:ring-secondary border-borderColor">
                                    <div class="ml-3">
                                ...
                                <label class="inline-flex items-center p-3 border border-borderColor rounded-lg hover:bg-gray-50 cursor-pointer transition-colors has-[:checked]:border-secondary has-[:checked]:bg-secondaryShades-100">
                                    <input type="radio" name="status" value="published" <?= ($blogPost['status'] === 'published') ? 'checked' : '' ?> class="h-4 w-4 text-secondary focus:ring-secondary border-borderColor">
                                    <div class="ml-3">
                                ...
                                <label class="inline-flex items-center p-3 border border-borderColor rounded-lg hover:bg-gray-50 cursor-pointer transition-colors has-[:checked]:border-secondary has-[:checked]:bg-secondaryShades-100">
                                    <input type="radio" name="status" value="archived" <?= ($blogPost['status'] === 'archived') ? 'checked' : '' ?> class="h-4 w-4 text-secondary focus:ring-secondary border-borderColor">
                                    <div class="ml-3">
                        ...
                        <div class="flex flex-col gap-3 w-full">
                            ...
                            <div class="py-3">
                                <div class="flex items-center">
                                    <input id="is_featured" name="is_featured" type="checkbox" <?= !empty($blogPost['is_featured']) ? 'checked' : '' ?> class="h-4 w-4 text-secondary focus:ring-secondary border-borderColor rounded">
                            ...
                            <div>
                                ...
                                <input type="number" id="reading_time" name="reading_time" min="1" value="<?= $blogPost['reading_time'] ?? 5 ?>" class="w-full px-4 py-3 border border-borderColor rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent">
        ...
        <div id="progressModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" style="display: none;">
            <div class="bg-white rounded-xl p-6 w-full max-w-md">
                <div class="flex flex-col items-center">
                    ...
                    <!-- Progress text -->
                    <h3 class="text-lg font-medium text-gray-900 mb-1">Updating Blog Post</h3>
                    <p class="text-sm text-gray-500 mb-4">Please wait while we update your blog post and upload files...</p>
...
<script>
    $(document).ready(function() {
        ...
        // Load existing tags from PHP - map tag IDs to tag names
        const selectedTagIds = <?= json_encode(array_column($postTags ?? [], 'tag_id')) ?>;
        const allTags = <?= json_encode(array_map(function($tag) { return ['id' => $tag['id'], 'name' => $tag['name']]; }, $tags)) ?>;

        // Initialize tags array with tag names from selected tag IDs
        let tags = selectedTagIds.map(tagId => {
        ...
        window.deleteExistingImage = function(imageId) {
            ...
            $(`.existing-image-item[data-image-id="${imageId}"]`).fadeOut(300, function() {
                ...
                if ($('#existing-images .existing-image-item').length === 0) {
                    $('#existing-images').parent().html('<p class="text-sm text-gray-500 mb-4">No existing images</p>');
                }
            });
        ...
        // Remove existing featured image function
        window.removeExistingFeaturedImage = function() {
            ...
            // Mark for removal
            $('#remove-featured-image').val('1');

            // Remove from DOM with animation
            $('#existing-featured-image').fadeOut(300, function() {
                $(this).remove();
            });

            showToast('Featured image marked for removal', 'info');
        };

        // Remove featured image preview (for newly uploaded images)
        ...
        // Initialize excerpt counter on page load
        const initialExcerptLength = $('#excerpt').val().length;
        $('#excerpt-count').text(`${initialExcerptLength}/1000`);
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        if (isDrafting) {
                            ...
                            progressModal.find('h3').text('Blog Post Updated!');
                        ...
                        // Show success toast
                        showToast(response.message || (isDrafting ? 'Draft saved successfully!' : 'Blog post updated successfully!'), 'success');
                        ...
                        setTimeout(() => {
                            if (response.redirect_url) {
                                ...
                                window.location.href = '<?= site_url('auth/blogs/blogs') ?>';
                            }
        ...
        $('#cancelUploadBtn').on('click', function() {
            ...
            // Re-enable form elements
            $('#editBlogForm').find('input, textarea, button, select').prop('disabled', false);
