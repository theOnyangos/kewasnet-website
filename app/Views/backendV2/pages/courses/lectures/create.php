...
<?= $this->section('content') ?>
    <main class="flex-1 overflow-y-auto main-content-area">
        <?= view('backendV2/partials/page_banner', [
            ...
            'pageDescription' => 'Create a new lecture for ' . $section['title'],
            'breadcrumbs' => [
        ...
        <div class="px-6 pb-6">
            <div class="bg-white rounded-xl shadow-sm max-w-full mx-auto">
                <form id="addLectureForm" class="p-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="lecture_title" name="title" placeholder="e.g., Understanding HTML Tags and Attributes" class="w-full px-4 py-3 border border-borderColor rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary" required>
                        ...
                        <div>
                            ...
                            <textarea id="lecture_description" name="description" rows="4" placeholder="What students will learn in this lecture..." class="w-full px-4 py-3 border border-borderColor rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary resize-none"></textarea>
                        ...
                        <div>
                            ...
                            <div id="resourceUrlsContainer" class="space-y-3">
                                <div class="resource-url-item flex gap-2">
                                    ...
                                    <button type="button" class="remove-resource-btn px-4 py-3 bg-red-50 border border-red-300 rounded-lg text-red-600 hover:bg-red-100 transition-colors" style="display: none;">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="flex flex-col gap-3 justify-center">
                                <label class="flex items-center cursor-pointer">
                                    <input type="checkbox" id="lecture_is_preview" name="is_preview" value="1" class="w-4 h-4 text-secondary border-gray-300 rounded focus:ring-2 focus:ring-secondary">
                                    <span class="ml-2 text-sm font-medium text-dark">Preview</span>
                                </label>
                                <label class="flex items-center cursor-pointer">
                                    <input type="checkbox" id="lecture_is_free" name="is_free_preview" value="1" class="w-4 h-4 text-secondary border-gray-300 rounded focus:ring-2 focus:ring-secondary">
...
$(document).ready(function() {
    ...
    $('#addLectureForm').on('submit', function(e) {
        ...
        submitBtn.prop('disabled', true).html('<i data-lucide="loader" class="w-4 h-4 inline mr-2 animate-spin"></i> Creating...');
        lucide.createIcons();

        $.ajax({
            url: '<?= site_url("auth/courses/lectures/create") ?>',
            method: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
            success: function(response) {
                if (response.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Success!',
                        text: response.message || 'Lecture created successfully',
            ...
            error: function(xhr) {
                let errorMsg = 'An error occurred while creating the lecture';
